...
{
	return (tree && tree->parent ? _binary_tree_depth(tree->parent) + 1 : 0);
}
		...
		const binary_tree_t *second)
{
	...
	current_f = (binary_tree_t *)first, current_s = (binary_tree_t *)second;
	...
	while (current_f && current_s)
	{
		if (current_f == current_s)
			return (current_f);
		current_f = current_f->parent, current_s = current_s->parent;
	}
	return (NULL);
}
